...
For automation, load `skills/opencart-deployment/SKILL.md` and use a target-version refresh contract/adapter in the actual target/tooling project.
...
- normalize old package XML to the canonical code before storing only when the project intentionally migrates away from versioned codes;
